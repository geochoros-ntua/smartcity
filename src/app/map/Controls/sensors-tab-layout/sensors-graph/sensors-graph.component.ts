...
export class SensorsGraphComponent implements OnInit {
  ...
  public graphTypes: { type: string; label: string }[] = [
    { type: 'bar', label: 'GRAPH.BAR' },
    { type: 'line', label: 'GRAPH.LINE' },
  ];
  public reportTypes: { type: string; label: string }[] = [
    { type: 'day', label: 'GRAPH.DAY' },
    { type: 'hour', label: 'GRAPH.HOUR' },
  ];
  ...
  public getCompareSensors(): FeatureLike[] {
    return this.mapLayersService.SensorsLayer.getSource()
      .getFeatures()
  ...
  public setComparingSensors(feats: FeatureLike[]): void {
    ...
    combineLatest(compareReports$).subscribe((result) => {
      result.forEach((cr, i) => {
        this.sensChart.data.datasets = [
          ...this.sensChart.data.datasets,
          {
            ...this.getStyleForGraph(i),
            label: sensorsLabels[i] + ' ' + this.translateLabel(),
            data: this.sensChart.data.labels.map((r: string) =>
              cr.map((rrr) => rrr.label).includes(r)
                ? cr.find((rrr) => rrr.label === r).value
                : 0
            ),
          },
        ];
  ...
  private handleCompareGrpahsAndUpdate(): void {
    if (this.graphForm.get('graphCompareSelector')?.value) {
      this.setComparingSensors(
        this.graphForm.get('graphCompareSelector')?.value
      );
    } else {
      this.sensChart.update();
    }
  ...
  public isDisabled(feat: FeatureLike): boolean {
    return (
      this.sensChart?.data.datasets.length > 5 &&
      !this.graphForm
        .get('graphCompareSelector')
        ?.value?.map((f: FeatureLike) => f.getId())
        .includes(feat.getId())
    );
  }
  ...
  private translateLabel(): string {
    return (
      this.sensorsService.translatePipe.transform('MAP.SENS-REPORT') +
      this.sensorsService.translatePipe.transform(
        this.reportTypes.find(
          (rt) => rt.type === this.sensorsService.selReportType
        ).label
      ) +
      ')'
    );

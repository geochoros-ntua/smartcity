...
import {
  SmartCityMapillaryConfig,
  SmartCityMapConfig,
  FeatureClickedWithPos,
} from '../api/map.api';
...
export class MapService {
    ...
    this.map = new Map({
      ...
      view: new View({
        center: olProj.transform(
          this.smartCityMapConfig.center,
          'EPSG:4326',
          'EPSG:3857'
        ),
  ...
  public flashFeature(feature: FeatureLike): void {
    ...
    const duration =
      featVal < intLimits[0]
        ? 3000
        : featVal < intLimits[1] && featVal > intLimits[0]
        ? 2000
        : 1000;
    const start = Date.now();
    const flashGeom: Geometry = (feature.getGeometry() as Geometry).clone();

    const listenerKey = this.mapLayersService.SensorsLayer.on(
      'postrender',
      (event: any) => {
  ...
  public onMapClicked(event: MapBrowserEvent<UIEvent>): void {
    ...
    if (this.mapMode === MapMode.stats) {
      const feature =
        this.mapLayersService.webGlStatsSource.getClosestFeatureToCoordinate(
          event.coordinate
        );
      if (!feature || this.mapStatsService.getFeatureVisiblity(feature) === 0)
        return;

      const featCenter = feature
        .getGeometry()
        ...
        this.tooltip.innerHTML =
          this.mapStatsService.selectedStatsIndex.type === StatTypes.number
            ? parseInt(
              ...
              1 +
              ''
            : this.mapStatsService.selectedStatsIndex.classes.find(
                (cls) =>
                  feature.get(this.mapStatsService.selectedStatsIndex.code) ===
                  cls.value
              )?.label;
          ...
          if (feature.get('layer')) {
            switch (feature.get('layer')) {
              ...
              case VectorLayerNames.point: {
                ...
                this.mapMapillaryService.showFeatureOnImage(
                  mapillaryViewerConfig,
                  feature
                );
                break;
              ...
              case VectorLayerNames.sens: {
                this.sensorsService.showReportGraph(
                  Number(feature.getId()),
                  Number(feature.get('live_report_id')),
                  feature.get('mpl_imageid')
                );
                break;
              }
              default: {
  ...
  private onModeChange(mode: MapMode): void {
    ...
    const msg =
      mode === 'street'
        ? this.translatePipe.transform('MAP.MAP-MODE', {
            msg: this.translatePipe.transform('MAP.MODE-MPLR'),
          })
        : mode === 'sens'
        ? this.translatePipe.transform('MAP.MAP-MODE', {
            msg: this.translatePipe.transform('MAP.MODE-SENSORS'),
          })
        : mode === 'stats'
        ? this.translatePipe.transform('MAP.MAP-MODE', {
            msg: this.translatePipe.transform('MAP.SUBJECTIVE-FACTORS'),
          })
        : 'Error: no such mode';
    ...
    switch (mode) {
      case MapMode.street: {
        this.mapLayersService.MlSequencesLayer.setVisible(
          this.mapLayersService.checkedSeq
        );
        this.mapLayersService.MlImagesLayer.setVisible(
          this.mapLayersService.checkedImg
        );
      ...
      case MapMode.stats: {
        ...
        this.mapLayersService.HeatMapLayer.setVisible(
          this.mapStatsService.heatEnable
        );
        this.mapLayersService.SensorsLayer.setVisible(false);
